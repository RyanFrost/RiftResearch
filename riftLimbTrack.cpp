...
string getJointAngles(vector< vector<double> >);
...
//-----------------------------------------------------------
	...
	while(runTheLoop)
	{
		...
		// This loop updates 'safe' marker arrays so that NaN values are not sent to rift
		for (int marker = 0; marker<10;marker++)
		{
			if ( !isnan(sdata->x_markers[marker]) )
			{
				xSafe[marker] = sdata->x_markers[marker];
			}
			if ( !isnan(sdata->y_markers[marker]) )
			{
				ySafe[marker] = sdata->y_markers[marker];
			}
			if ( !isnan(sdata->z_markers[marker]) )
			{
				zSafe[marker] = sdata->z_markers[marker];
			}
			//cout << marker+1 << ": - x: " <<xSafe[marker] << ", y: " << ySafe[marker] << ", z: " << zSafe[marker] << endl;
		}
		
		string jointString;
		
		vector< vector<double> > markerVec1 = leg_tracking(xSafe, ySafe, zSafe);
		
		if (markers.size >5)
		{
			vector< vector<double> > markerVec2 (5,vector<double>(3, 0.0));
			for (int it = 0; it<5;it++)
			{				
				markerVec2[it] = markerVec1[it+5];
			}
			
			markerVec1.erase(markerVec1.begin()+5,markerVec1.end());
			
			jointString.append(getJointAngles(markerVec1));
			jointString.append(getJointAngles(markerVec2));
		}
		else
		{
			jointString.append(getJointAngles(markerVec1));
		...
		char *jointChar = new char[jointString.size()+1];
		jointChar[jointString.size()] = 0;
		memcpy(jointChar,jointString.c_str(),jointString.size());
...
vector< vector<double> > leg_tracking(double xMarkers[], double yMarkers[], double zMarkers[])
{
	...
	vector< vector<double> > markerArray (numMarkers, vector<double> (4,0.0));
	vector< vector<double> > markerArrayTransf (numMarkers, vector<double> (4,0.0));
	...
	for (int markerNum = 0;markerNum<numMarkers;markerNum++)
	{
		for (int i = 0; i < 4; i++)
		{
			for (int j = 0; j < 4; j++)
	...
	for (int markerIt = 0; markerIt<numMarkers;markerIt++)
	{
...
string getJointAngles ( vector< vector<double> > markerVecs)
{
	
	
	double dubTH[] = { 0.0, -1.0};
	double dubHK[] = { markerVecs[2][0]-markerVecs[1][0], markerVecs[2][1]-markerVecs[1][1] };
	double dubKA[] = { markerVecs[3][0]-markerVecs[2][0], markerVecs[3][1]-markerVecs[2][1] };
	double dubAT[] = { markerVecs[4][0]-markerVecs[3][0], markerVecs[4][1]-markerVecs[3][1] };
	
	vector<double> torsoHip (dubTH, dubTH +sizeof(dubTH)/sizeof(double));
	vector<double> hipKnee (dubHK, dubHK +sizeof(dubHK)/sizeof(double));
	vector<double> kneeAnkle (dubKA, dubKA +sizeof(dubKA)/sizeof(double));
	vector<double> ankleToe (dubAT, dubAT +sizeof(dubAT)/sizeof(double));
	
	vector< vector<double> > vecs;
	vecs.push_back(torsoHip);
	vecs.push_back(hipKnee);
	vecs.push_back(kneeAnkle);
	vecs.push_back(ankleToe);
	...
	for ( int vecIt = 0; vecIt < 3; vecIt++)
	{
		...
		angles[vecIt] = atan2(det, dot) * 180 /PI;
	}
