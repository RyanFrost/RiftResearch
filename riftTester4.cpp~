...
int main (int argc, char *argv[])
{
	...
	int patchBufferStart = 3; // # of patches before the first perturbation
...
*/
				...
				/*for (bufIter = 0; bufIter<STEPSIZE; bufIter++)
				{
					stepData[bufIter] = 0;
				}
				*/
			...
			bool move_for_pert = FALSE;
			if (move_for_pert && flag == 0)
			{
				sdata->Kd = 50000;
				/* ----( Controller )---- */
				// Calculate actual stiffness
				sdata->Kact = (xb*sdata->force_b) / (pow( (sdata->foot_pos_x/100.0) ,2.0)*tan(sdata->angle_enc*M_PI/180));
		...
		// end of "flag == 0"				//dufb = (-1.0)*(Kp*err - Kp*errp + Ki*err*sdata->elapsed);
					}
					else
					{
						ufb = 0;			
			...
			try
			{
				...
				if(sendto(sockfd, "Running", STEPSIZE, 0, (struct sockaddr *)&remote_addr, addrlen) < 0)
				{
					perror("Send failed.");
					//close(sockfd);
					return 0;
