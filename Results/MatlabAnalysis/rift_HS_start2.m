...
th_hip = zeros(1,length(hip));
th_knee = zeros(1,length(knee));
th_ankle = zeros(1,length(ankle));
...
for i = 3:length(heel_strike_ind) - 2
    range = heel_strike_ind(i-2) : heel_strike_ind(i+2); % take 1 gait cycle
...
for i = 1+numCyclesBefore:num_inf_cycs-numCyclesAfter-1
    ...
    infplots(i)=plot(ankle_angle_inf{i},'tag',num2str(ind1));hold on;    
...
ankle_infStd = std(cell2mat(ankle_angle_inf));
knee_infAvg = mean(cell2mat(knee_angle_inf));
knee_infStd = std(cell2mat(knee_angle_inf));
hip_infAvg = mean(cell2mat(hip_angle_inf));
hip_infStd = std(cell2mat(hip_angle_inf));
...
for i = 1:numCyclesBefore
    ankle_inf = [ankle_inf, ankle_infAvg];
    ankleStd = [ankleStd, ankle_infStd];
    knee_inf = [knee_inf, knee_infAvg];
    kneeStd = [kneeStd, knee_infStd];
    hip_inf = [hip_inf, hip_infAvg];
    hipStd = [hipStd, hip_infStd];
end
...
for cycle = 1+numCyclesBefore : length(heel_strike_ind) - numCyclesAfter -2
    hsInd = heel_strike_ind(cycle);
        
    noPertRange = [heel_strike_ind(cycle-numCyclesBefore:cycle-1);heel_strike_ind(cycle+1:cycle+numCyclesAfter+2)];
    
    if perturb(hsInd) == 1 && Kd(hsInd) == 6e4 && isempty(find(perturb(noPertRange)>0,1,'first'))
        
        type_w_inds{1}{end+1} = heel_strike_ind(cycle - numCyclesBefore) : heel_strike_ind(cycle + 2 + numCyclesAfter);
    
    elseif perturb(hsInd) == 2 && Kd(hsInd) == 1e6 && isempty(find(perturb(noPertRange)>0,1,'first'))
        
        type_w_inds{2}{end+1} = heel_strike_ind(cycle - numCyclesBefore) : heel_strike_ind(cycle + 1 + numCyclesAfter);
    
    elseif perturb(hsInd) == 3 && Kd(hsInd) == 6e4 && isempty(find(perturb(noPertRange)>0,1,'first'))
...
for i = 1:num_types_perts
    for j = 1:length(type_w_inds{i})
        ...
        if(i==1)
            kneePlots(j) = plot(knee_angle{i}{j},'tag',num2str(data(1)));hold on;    
...
%%%%%%% CHANGE      i = 1:2 to i = 1:num_types_perts    TO INCLUDE TYPE 1
%%%%%%% PERTURBATIONS
...
ankle{4} = [ankle_inf; ankleStd];
knee{4} = [knee_inf; kneeStd];
hip{4} = [hip_inf; hipStd];
...
lengthVec = 1:indEnd;
figure;
plotyy(lengthVec,xf',lengthVec,perturb); hold on;
for i = 1:length(heel_strike_ind)
    plot([heel_strike_ind(i), heel_strike_ind(i)], [10,70],'-k');
end
