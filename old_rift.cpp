...
*/
				...
				if ( fabs(xf-xmid+2) <= 5.0 && progress > 0 ) // xf decreasing (swing phase) 
				{
					...
					if (cycle == 10 || cycle == 60 || cycle == 90  )
					sdata->beep = TRUE;

	
				} // end of "if ( fabs(xf-xmid) <= 5 && progress > 0 )"
				else if ( fabs(xf-xmid+2) >= 10.0)
				{
				range_num = 0;
				}
				...
				for (int bufIter = n; bufIter<STEPSIZE; bufIter++)
				{
					stepData[bufIter] = 0;
				}
				cout << stepData << "\n";
				...
				for (int bufIter = 0; bufIter<STEPSIZE; bufIter++)
				{
					stepData[bufIter] = 0;
				}
			...
			fprintf(fp,"%f %f %d %d %f %f %f %d %f %d %f %d\n",stepsTaken,aveStepLength,sdata->cycle,toeoff,xf,length,sdata->elapsed, count,frac, reset_val, ave_reset_val,sand);
			sleep(.001); //???
		}
