...
//********************************
	while( true )
	{
		...
		{	
			...
			while(!(movingForward || movingBackward))
			{
				movingForward = posData.forward;
				movingBackward = posData.backward;
			}
			
			if ( movingForward && posData.xf > 30.0; )
			{
				while(posData.forward){}
			}
			
			while(!posData.heelStrike){}
			
			stiffness = 6e4;
			
			while(!posData.cycled)
			{
				moveTrack;
			}
...
*/
			vec = pastVals;
			vector<double> vecReverse(vec.rbegin(),vec.rend());
...
// 						vecReverse[0],vecReverse[1],vecReverse[2]);
			...
			if (is_sorted(vecReverse))
			{
				if(movingBackwardFirst)
				{
					movingForward = true;
					movingBackward=false;
			...
			if (flag==0 && changedStiff) // No nan problems for xf and xmid
			{
