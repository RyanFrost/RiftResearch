...
% subject specific
if strcmp(subject,'jeremy')
    thresh1 = .85;
    thresh2 = .8;
    thresh3 = .7;
    stiffnessVals = [.0959 .0578  .0275];
elseif strcmp(subject,'dave')
    thresh1 = .8;
    thresh2 = .8;
    thresh3 = .8;
    stiffnessVals = [.2433 .0875  .0192];
elseif strcmp(subject,'josh')
    thresh1 = .5; % also removed {1}{1} and {3}{3}
    thresh2 = .4;
    thresh3 = .3;
    stiffnessVals = [.2694 .0347  .0157]; 
else
    disp('Who is the subject?');
    return
end

A = load([subject,'_july14.txt']); % main data
B = load([subject,'_july14.emg']); % EMG data
...
% iddata - create individual models (need output and input vectors)
%aveTimePerCycle = 1.08; % s

% prepend data???
%[zeros(nx,1); ones(N,1)];
% 
% Warning: For transient data (step or impulse experiment), make sure that the change
% in input signal does not happen too early relative to the order of the desired
% model. You can achieve this by prepending sufficient number of zeros (equilibrium
% values) to the input and output signals. For example, a step input must be
% represented as [zeros(nx,1); ones(N,1)] rather than ones(N,1), such that nx > model
% order. 

clear dataTa

Ts = aveTimePerCycle/length(xx); % seconds/sample
dataTa = iddata(ta{1}',mag{1}',Ts);

% merge - into dataAll
for i = 2:numCyclesTot  
    dataTa = merge( dataTa, iddata(ta{i}',mag{i}',Ts));
end

% filter
temp = idfilt(dataTa,[0 100],'noncausal');
clear dataTa
dataTa = temp;
clear temp

figure;
plot(getexp(dataTa,1));

%%

% For a single-output system, this is a column vector. 
% For a multiple-output system with Ny output channels and NT time samples,
% this is an NT-by-Ny matrix.

% For a single-input system, this is a column vector.
% For a multiple-input system with Nu input channels and NT time samples,
% this is an NT-by-Nu matrix.

% getexp - randomly select data to be train and test groups
randMat = randperm(numCyclesTot);
n = round(0.8*numCyclesTot);

dataTrainTa = getexp(dataTa,randMat(1:n));

dataTestTa = getexp(dataTa,randMat(n+1:end));


% preliminary delay estimate na = nb = 2
delayEstimate = delayest(dataTrainTa);

%% model order estimate

% NN1 = struc(2:5,1:5,delayEstimate-5:delayEstimate+5);
% nn = selstruc(arxstruc(dataTrainTa(:,:,1),dataTrainTa(:,:,1),NN1) ,0);
% na = nn(1);
% nb = nn(2);
% nk = nn(3);

data = dataTrainTa;
%% black-box, input-output polynomial models
order = 2;
na = order;
nb = order;
nc = order;
...
% nk = 5 % delay taken out above

% polyest
%mpoly = polyest(data, [na nb nc nd nf nk]);
% oe
%moe = oe(data,[nb nf nk]);
% Box-Jenkins
mbj = bj(data,[nb nc nd nf nk]);

disp('models created')
%%
%compare(dataTestTa,moe,mbj);
compare(dataTestTa,mbj);
disp('compared')
return
%%
% [y,fit,x0]=compare(dataTestTa,moe,mbj);
%[y,fit,x0] = compare(dataTestTa,mbj);
save([subject,'_mbj'],'dataTestTa','mbj');
